...
Ein E-Commerce-Webshop (SPA) mit React, TypeScript, Vite und Tailwind CSS. Das Node.js/Express-Backend kommuniziert über eine REST API mit iDempiere ERP.

> Ausführliche Architektur, Datenfluss-Diagramme und API-Referenz: [ARCHITECTURE.md](ARCHITECTURE.md)
...
npm install
cp .env.example .env
# .env mit echten Werten füllen (siehe Abschnitt Konfiguration)
```
...
Benötigte Apache-Module: `mod_proxy`, `mod_proxy_http`, `mod_rewrite`.
...
# Prozessliste speichern (für Autostart)
pm2 save

# Autostart bei Reboot einrichten (einmalig)
pm2 startup systemd
# Den ausgegebenen sudo-Befehl ausführen, dann:
pm2 save
```
...
- [ ] `JWT_SECRET`: min. 32 Zeichen, kryptografisch zufällig
- [ ] CORS-Origins in `server/index.js` auf Produktions-Domain ändern
...
Erstelle `.env` im Wurzelverzeichnis. Siehe `.env.example` für die vollständige kommentierte Vorlage.
...
Alle weiteren Variablen haben sinnvolle Defaults in `server/config.js` und müssen nur gesetzt werden, wenn man vom Default abweichen will:

| Variable | Default | Zweck |
|---|---|---|
| `PORT` | 3001 | Backend-Port |
| `IDEMPIERE_CLIENT_ID` | 11 | iDempiere Mandant (GardenWorld) |
| `IDEMPIERE_ROLE_ID` | 102 | iDempiere Rolle für Admin-Token |
| `IDEMPIERE_ORG_ID` | 11 | Organisation |
| `IDEMPIERE_WAREHOUSE_ID` | 1000000 | Lager-ID (für Orders) |
...
| `IDEMPIERE_REGISTRATION_ROLE_ID` | 1000000 | Rolle für neue User |
...
- Alle iDempiere-IDs müssen zur jeweiligen Installation passen

Für API-Referenz, Architektur und Troubleshooting: [ARCHITECTURE.md](ARCHITECTURE.md)
